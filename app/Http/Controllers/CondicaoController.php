...
class CondicaoController extends Controller
{
    ...
    {
        $grupo_id[0] = $grupo_id;
        return view('condicaos.create', ['produtos' => Produto::all(), 'parametros' => Parametro::all(), 'grupo_id' => $grupo_id]);
    }
    ...
    public function edit(Condicao $condicao)
    {
        ...
        $parametroSelecionado = Parametro::find($condicao->parametro_id);

        $parametros = array();
        foreach(Parametro::all() as $parametro){
          if($parametroSelecionado->id != $parametro->id){
            $parametros[] = $parametro;
          }
        }


        return view('condicaos.edit', compact('condicao', 'produtoSelecionado', 'produtos', 'parametroSelecionado', 'parametros'));
    }
